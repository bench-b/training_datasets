...
from hyperopt import fmin, tpe, hp, Trials, STATUS_OK #hyperparameter hyperopt for randomforest
...
# search space (keep values realistic to control time)
rf_space = {
    "n_estimators": hp.quniform("n_estimators", 300, 800, 50),
    "max_depth": hp.choice("max_depth", [5, 10, 15, 20]),
    "min_samples_split": hp.quniform("min_samples_split", 5, 30, 5),
...
def rf_objective(params):
    ...
    # Use AUC (better for imbalance) — we *minimize* loss, so use 1 - mean_score
...
best_rf = fmin(
    ...
    space=rf_space,
    algo=tpe.suggest,
    max_evals=100,   # increase if you have time
    trials=rf_trials,
    rstate=np.random.default_rng(42)
)

# decode choice indices
max_depth_choices = [None, 5, 10, 15, 20, 30]
max_features_choices = ["sqrt", "log2", None]

best_rf_params = {
    "n_estimators": int(best_rf["n_estimators"]),
    "max_depth": max_depth_choices[best_rf["max_depth"]],
...
# Train the best RF on the full (SMOTE) training set and evaluate on holdout test
best_rfc = RandomForestClassifier(**best_rf_params)
